	...
	implements Comparable<Entity>
{
	...
	public Image getImage()
	{
		...
		if (selected)
		{
			BufferedImage buff = new BufferedImage(xs, ys, BufferedImage.TYPE_INT_ARGB);
			Graphics g = buff.getGraphics();
			g.drawImage(n, 0, 0, null);
			Color c = Color.GREEN;
			g.setColor(c);
			g.drawRect(0, 0, buff.getWidth(), buff.getHeight());
			n = buff;
		}
		
		
		if (newXs > 0 && newYs > 0)
			return n.getScaledInstance(newXs, newYs, Image.SCALE_DEFAULT);
		else
			return n.getScaledInstance(xs, ys, Image.SCALE_DEFAULT);
	}
